...
<form method="post" action="<?php echo $controller->action('submit')?>" data-dialog-form="search-customize">
	...
	<fieldset>
		...
		<?php foreach ($fldc->getColumns() as $col) { ?>
			<li style="border:1px dotted #ccc; margin-top:4px; cursor: move" data-field-order-column="<?php echo $col->getColumnKey()?>"><input type="hidden" name="column[]" value="<?php echo $col->getColumnKey()?>" /><?php echo $col->getColumnName()?></li>
		<?php } ?>
		</ul>
	</fieldset>
