...
class Order
{
    ...
    /** @Column(type="decimal", nullable=true, precision=10, scale=2) **/
    protected $oPaidTotal;
   
    /** @Column(type="decimal", nullable=true, precision=10, scale=2) **/
    protected $oRefundTotal;
    
    /** @Column(type="datetime", nullable=true) */
    protected $oPaidDate;
    
    /** @Column(type="datetime", nullable=true) */
    protected $oPaidExpiredDate;
    ...
    {
        $this->oDiscountTotal = $discountTotal;
    }
    ...
    {
        $this->oPaidTotal = $oPaidTotal;
    }
    public function setOrderRefundTotal($oRefundTotal)
    {
        $this->oRefundTotal = $oRefundTotal;
    }
    ...
    {
        $this->oPaidExpiredDate = $oPaidExpiredDate;
    }
    ...
    public function add($data, $pm, $transactionReference='', $status=null)
    {
        ...
        $smName = StoreShippingMethod::getActiveShippingMethodName();
        $shippingTotal = StoreCalculator::getShippingTotal();
		$discountTotal = StoreCalculator::getTotalDiscount();
        $taxes = StoreTax::getConcatenatedTaxStrings();
        $total = StoreCalculator::getGrandTotal();

        $order = new Order();
        $order->setCustomerID($dataAddress['baID']);
        $order->setOrderDate($now);
        $order->setPaymentMethodName($pmName);
        $order->setShippingMethodName($smName);
        $order->setShippingTotal($shippingTotal);
		...
		$start = date_format($now,"Y-m-d H:i:s");
		$year = 0;
		$month = 0; 
		$day = Config::get('vividstore.paymentTimingDays');
		$hour = Config::get('vividstore.paymentTimingHours');;
		$minutes = Config::get('vividstore.paymentTimingMinutes');;
		$seconds = Config::get('vividstore.paymentTimingSeconds');;
		$paidExpiredDateTime = date('Y-m-d H:i:s',strtotime('+'.$year.' year +'.$month.' month +'.$day.' day +'.$hour.' hour +'.$minutes.' minutes +'.$seconds.' seconds',strtotime($start)));
        $order->setOrderPaidExpiredDate(new \DateTime($paidExpiredDateTime));

        $order->save();

		$discountsWithCodesExist = StoreDiscountRule::discountsWithCodesExist();
		if ($discountsWithCodesExist) {
			$order->addDiscount(StoreCart::getDiscounts(), $order->getOrderID());
		}
	...
	public static function getForCurrentOrder($oID)
    {
        $db = Database::get();
		$dataAddress = AddressModal::getMyAddressByDefault();
				
		$v = array($oID, $dataAddress['baID']);
		$hasOrder = $db->GetOne("SELECT count(oID) FROM vividstoreorders WHERE oID = ? AND cID = ? ", $v);
    ...
    public function completePayment($oID = null, $pmHandle = null, $statusPaid = null, $paidTotal = null, $paidDate = null, $paidTime = null, $refundTotal = null, $transactionReference = null)
    {
        ...
        //} else {
            //$tr = new self();
        //}
        $tr->setPaymentMethodName($pmHandle);
        $tr->setOrderPaid($statusPaid);
        $tr->setOrderPaidTotal($paidTotal);
		$paidDate = str_replace('/', '-', $paidDate);
		$paidDateTime = date("Y-m-d H:i", strtotime($paidDate.' '.$paidTime));
        $tr->setOrderPaidDate(new \DateTime($paidDateTime));
		$tr->setOrderRefundTotal($refundTotal);
        $tr->setTransactionReference($transactionReference);
        $tr->save();

        $tr->updateStatus();
		
        $tr->dispatchEmailNotifications();
        StoreCustomer::addCustomerToUserGroupsByOrder($tr);
    ...
    public function addDiscount($discounts, $orderID)
    {
		...
		foreach ($discounts as $discount) {
			$vals = array($orderID,$discount->drName, $discount->getDisplay(), $discount->drValue,$discount->drPercentage, $discount->drDeductFrom, $discount->dcCode);
