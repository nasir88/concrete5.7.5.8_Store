...
class OrderItem
{
    ...
    public function add($data, $oID, $tax = 0, $taxIncluded = 0, $taxName = '')
    {
		...
		if ($variation) {
			$pvID = $data['product']['variation'];
            if (!$variation->isUnlimited()) {
                $product->updateProductQty($newStock,$pvID);
            }
        } elseif (!$product->isUnlimited()) {
            $product->updateProductQty($newStock,NULL);
        }
		...
		if ($variation) {
			$orderItem->setProductVariationID($pvID);
		}else{
			$orderItem->setProductVariationID(0);
		}
		//End
