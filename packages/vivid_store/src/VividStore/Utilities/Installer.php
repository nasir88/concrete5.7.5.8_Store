...
class Installer
{
    public static function installSinglePages(Package $pkg)
    {
        ...
        Installer::installSinglePage('/payment', $pkg);
        Page::getByPath('/payment/')->setAttribute('exclude_nav', 1);
		//End
		//Add By Nasir - MYORDERS001
        Installer::installSinglePage('/order/', $pkg);
        Page::getByPath('/order/')->setAttribute('exclude_nav', 1);
        Installer::installSinglePage('/order/my_orders', $pkg);
        Page::getByPath('/order/my_orders')->setAttribute('exclude_nav', 1);
        Installer::installSinglePage('/order/my_coupons', $pkg);
        Page::getByPath('/order/my_coupons')->setAttribute('exclude_nav', 1);
        Installer::installSinglePage('/order/my_refunds', $pkg);
        Page::getByPath('/order/my_refunds')->setAttribute('exclude_nav', 1);
		//End
    ...
    public static function installPaymentMethods(Package $pkg)
    {
		...
		//Add By Nasir - PAYMENT001
        Installer::installPaymentMethod('bank_transfer', 'Bank/ATM Transfer Form', $pkg);
		//End
    }
	...
	//Add By Nasir - MODIFYDB001
	public static function modifyDatabaseTables($pkg)
    {
		$db = Database::get();
		
		//Add By Nasir - SHIPPING001
		//$databaseName = \Config::get('database.connections.concrete.database');
		//$rows = $db->GetAll("SELECT COUNT(*) AS Count FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='".$databaseName."' AND TABLE_NAME='VividStoreFlatRateMethods' AND COLUMN_NAME = 'postalSelected';");
		...
		//if ($count == 0) {
		//	$db->Execute("ALTER TABLE VividStoreFlatRateMethods ADD postalSelected LONGTEXT");
		//}
		//End
