...
class Product
{
	...
	public function updateProductQty($qty,$pvID = null)
    {
        if ($this->hasVariations()) {
			...
			$variation = StoreProductVariation::getByID($pvID);
            if ($variation) {
                $variation->setVariationQty($qty);
                $variation->save();
            }
    ...
    {
        return
		//Edit By Nasir - ERROR001
        $saleprice = StorePrice::format($this->getProductSalePrice());
    ...
    public function getDimensions($whl=null)
    {
        ...
        switch ($whl) {
            ...
            case "l":
                ...
                break;
            default:
				//Edit By Nasir - ERROR002
				if ($this->hasVariations() && $variation = $this->getVariation()) {
					return $source->getVariationLength()." x ".$source->getVariationWidth()." x ".$source->getVariationHeight();
				}else{
