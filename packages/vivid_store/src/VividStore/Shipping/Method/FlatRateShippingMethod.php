...
class FlatRateShippingMethod extends StoreShippingMethodTypeMethod
{
    ...
    {
        $this->postalSelected = $postalSelected;
    }
	//End
    ...
    private function addOrUpdate($type, $data)
    {
		...
		//Add By Nasir - (SHIPPING001)
        $sm->setPostalSelected($data['tagsPostcode']);
		//End
        ...
        if ($this->isWithinRange()) {
            if ($this->isWithinSelectedCountries()) {
                if ($this->isWithinWeight()) {
					...
					//Add By Nasir - (SHIPPING001)
					//return true;
					if ($this->isWithinSelectedPostal()) {
						return true;
					}else{
						return false;
					}
    ...
    public function isWithinSelectedCountries()
    {
        ...
        if ($this->getCountries() != 'all') {
            ...
            if (in_array($dataAddress['baCountry'], $selectedCountries)) {
                return true;
            } else {
                return false;
            }
	...
	public function isWithinSelectedPostal()
    {
        ...
        if ($this->getPostalSelected() != '') {
            $selectedPostal = explode(',', $this->getPostalSelected());
            if (in_array($dataAddress['baPostalCode'], $selectedPostal)) {
                return true;
            } else {
                return false;
            }
