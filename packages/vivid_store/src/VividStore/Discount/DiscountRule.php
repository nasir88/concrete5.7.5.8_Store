...
class DiscountRule extends Object
{
	...
	public static function getByDisplay($drDisplay)
    {
        $db = Database::get();
        $data = $db->GetRow("SELECT * FROM VividStoreDiscountRules WHERE drDisplay=?",$drDisplay);
        return self::load($data);
    }
