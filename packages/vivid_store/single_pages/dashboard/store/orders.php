...
<?php  if ($controller->getTask() == 'order') {
    ...
    if ($orderemail) {
        ?>
            ...
            <p><a href="mailto:<?php echo $order->getAttribute("email");
        ?>"><?php echo $order->getAttribute("email");
        ?></a></p>
            <?php  
    }
    ?>
    ...
    if ($ui) {
        ?>
            ...
            <p><a href="<?php echo  View::url('/dashboard/users/search/view/' . $ui->getUserID());
    ...
    if ($items) {
        foreach ($items as $item) {
            ?>
                <tr>
                    <td><?php echo $item->getProductName()?>
    ...
    ?>
                <tr>
                    <td><?php echo  h($discount['odName']);
    ?></td>
                    <td><?php echo  h($discount['odDisplay']);
    ?></td>
                    <td><?php echo  h($discount['odDeductFrom']);
    ?></td>
                    <td><?php echo  ($discount['odValue'] > 0 ? $discount['odValue'] : $discount['odPercentage'] . '%');
    ?></td>
                    <td><?php echo  ($discount['odCode'] ? t('by code'). ' ' .$discount['odCode']: t('Automatically'));
    ?></td>
                </tr>
            <?php  
}
    ...
    if ($transactionReference) {
        ?>
             <strong><?php echo t("Transaction Reference")?>: </strong><?php echo $transactionReference?><br>
        <?php  
    }
    ?>
        <?php  if ($order->isShippable()) {
    ?>
        <br /><strong><?php echo t("Shipping Method")?>: </strong><?php echo $order->getShippingMethodName()?>
        <?php  
}
    ?>
    </p>
...
<div class="ccm-dashboard-content-full">
    ...
    ?>
                        <li><a href="<?php  echo View::url('/dashboard/store/orders/', $status->getHandle())?>"><?php echo $status->getName();
    ?></a></li>
                    <?php  
}
    ?>
        ...
        <tbody>
            <?php 
                ...
                <tr>
                    ...
                    <td><?php echo $dh->formatDateTime($order->getOrderDate())?></td>
