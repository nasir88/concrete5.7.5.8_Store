...
<div class="vivid-store-utility-links <?php echo  ($itemCount == 0 ? 'vivid-cart-empty' : '');?>">
    ...
    <?php  if ($popUpCart) {
    ?>
    <a href="javascript:vividStore.displayCart()" class="cart-link"><?php echo $cartLabel?></a>
    <?php  
} else {
    ?>
    <a href="<?php echo View::url('/cart')?>" class="cart-link"><?php echo $cartLabel?></a>
    <?php  
} ?>

    <?php  if ($showCheckout) {
    ?>
    <a href="<?php echo View::url('/checkout')?>" class="cart-link"><?php echo t("Checkout")?></a>	
    <?php  
} ?>
