...
$groupViews = array('groups','groupadded','addgroup','savegroup');
...
<?php if (in_array($controller->getTask(), $addViews)) { //if adding or editing a product
 ...
 ?>
    ...
    <?php if ($pID > 0) {
    ?>
    <div class="ccm-dashboard-header-buttons marginBot10">
        <form method="post" id="delete" action="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/products/delete/', $pID)?>" >
            <button class="btn btn-danger">
				<i class="fa fa-trash"></i> &nbsp;
				<?php echo t("Delete Product")?>
			</button>
        </form>
    ...
    ?><br>
            ...
            <div class="col-sm-8 store-pane" id="product-categories">
				<?php echo $form->label("pCategorized", t("Categorized under pages")); ?>
    ...
    ?>
				...
				<?php echo $form->label("pInproduct", t("In product groups")); ?>
    ...
    ?>
				...
				<?php echo $form->label("pAdditionalImages", t("Additional Images"), array('style' => 'text-decoration:underline; margin-bottom:5px;')); ?>
                <div id="additional-images-container"></div>

                <div class="clearfix">
                    <span class="btn btn-success" id="btn-add-image">
    ...
    ?>
            ...
            <div class="col-sm-8 store-pane" id="product-options">
				...
				<?php echo $form->label("pOptions", t("Options"), array('style' => 'text-decoration:underline; margin-bottom:5px;')); ?>
                <div id="product-options-container"></div>

                <div class="clearfix">
                    <span class="btn btn-success" id="btn-add-option-group">
						<i class="fa fa-plus-square"></i> &nbsp;
						<?php echo t('Add Option Group')?>
					</span>
                </div>
    ...
    ?>
                    <div class="alert alert-info">
                        <?php
                        $a = '<a href="'.URL::to('/'.$_SESSION['myURLTheme'].'/system/permissions/advanced').'"><strong>';
    $aa = '</strong></a>';
    ...
    if (!$page->isError()) {
        ...
        ?>
                        ...
                        <a href="<?php echo Url::to('/'.$_SESSION['myURLTheme'].'/store/products/generate/', $p->getProductID())?>" class="btn btn-primary" id="btn-generate-page"><?php echo t("Generate a Product Page")?></a>
    ...
    ?>
        ...
        <div class="ccm-dashboard-form-actions-wrapper">
            <div class="ccm-dashboard-form-actions">
                <a href="<?php echo URL::to('/'.$_SESSION['myURLTheme'].'/store/products/')?>" class="btn btn-default pull-left mobile-width"><?php echo t("Cancel / View All Products")?></a>
                <button class="pull-right btn btn-primary mobile-width mobile-marginTop10" disabled="disabled" type="submit" ><?php echo t('%s Product', $actionType)?></button>
            </div>
        </div>
...
<?php 
	...
	<form role="form" class="form-inline ccm-search-fields">
		<div class="ccm-search-fields-row">
			...
			<div class="row">
				<div class="col-nsr-100 padLeftRight15 marginBot10">
					
					<div class="input-group pull-right mobile-width mobile-pull-right">
						<a class="btn btn-success pull-right mobile-width" href="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/products/', 'add')?>">
							<i class="fa fa-plus-square"></i> &nbsp; 
							<?php echo t("Add Product")?>
						</a>	
					</div>
				
					<div class="input-group pull-right marginRight05 mobile-width mobile-pull-right mobile-marginTop10">
						<a class="btn btn-dark pull-right mobile-width" href="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/products/', 'groups')?>">
							<i class="fa fa-cog"></i> &nbsp;
							<?php echo t("Manage Groups")?>
						</a>
					</div>
			...
			<div class="row">
				<div class="col-nsr-100 padLeftRight15">
					<ul id="group-filters" class="nav nav-pills store_group_filter">
						<li><a href="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/products/')?>"><?php echo t('All Groups')?></a></li>
						<?php foreach($grouplist as $group){ ?>

							<li><a href="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/products/', $group->getGroupID())?>"><?php echo $group->getGroupName()?></a></li>
						<?php } ?>
					</ul>
				</div>
			</div>
    ...
    foreach ($products as $p) {
        ...
        ?></td>
                        <td><strong><a href="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/products/edit/', $p->getProductID())?>"><?php echo  $p->getProductName();
        $sku = $p->getProductSKU();
        if ($sku) {
            echo ' (' .$sku . ')';
        }
        ...
        ?>
                        ...
                        <td>
                            <a class="btn btn-default"
                               href="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/products/edit/', $p->getProductID())?>"><i
...
<?php 
	...
	<form name="frmSection" id="frmSection" enctype="multipart/form-data" method="post" action="<?php echo $view->action('savegroup')?>" data-save-url="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/products/savegroup')?>" msg-delete-url="<?php echo t("Are You Sure?") ?>" data-delete-url="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/products/deletegroup')?>">
	
		<div id="errors2"></div>
	
		<div class="row">
			<div class="col-nsr-100 padLeftRight15">
				<div class="input-group mobile-width mobile-pull-left">
					<a class="btn btn-success mobile-width" onclick="addNameSection()">


						<i class="fa fa-plus-square"></i> &nbsp;
						<?php echo t("Add Group") ?>
					</a>
				</div>
			</div>
		</div>
		...
		<div id="nameBoxWrap">
			...
			?>
				...
				<div id="nameBox<?php echo $index ?>" class="row marginTop05">
					<div class="col-md-6">
						<div class="input-group">
							<input type="hidden" name="getGroupID" id="getGroupID<?php echo $index ?>" value="<?php echo $group->getGroupID()?>">
							<input type="text" class="form-control" name="name[]" id="name<?php echo $index ?>" value="<?php echo $group->getGroupName()?>">
							<span class="input-group-btn">
								<span class="btn btn-danger" onclick=removeNameSection("<?php echo $index ?>")><i class="fa fa-trash trash<?php echo $index ?>"></i><i class="fa fa-spinner spinner<?php echo $index ?> fa-spin" style="display:none"></i></span>
							</span>
						</div>
					</div>
				</div>
		...
		<div class="ccm-dashboard-form-actions-wrapper">
            <div class="ccm-dashboard-form-actions">
                <a href="<?php echo URL::to('/'.$_SESSION['myURLTheme'].'/store/products/')?>" class="btn btn-default pull-left mobile-width mobile-marginTop10"><?php echo t("Cancel / View All Products")?></a>
				<input type="submit" value="<?php echo t('Save');?>" class="btn btn-primary pull-right mobile-width mobile-marginTop10">
				<!--<input type="button" name="submit" value="<?php echo t('Save');?>" onclick="submitSection()" class="btn btn-primary">-->
				<input type="hidden" id="addSectionCount" value="<?php echo $index; ?>" name="addSectionCount">
            </div>
        </div>
