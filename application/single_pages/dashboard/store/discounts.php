...
<?php if (in_array($controller->getTask(), $listViews)){ ?>
		...
		<div class="row">
			<div class="col-lg-12">
				...
				<div class="input-group pull-right mobile-width mobile-pull-right">
					<a class="btn btn-success pull-right mobile-width" href="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/discounts/', 'add')?>">
						<i class="fa fa-plus-square"></i> &nbsp; 
						<?php echo t("Add Discount Rule")?>
					</a>	
				</div>	

				<div class="input-group width40 mobile-width pull-left mobile-pull-left mobile-marginTop10">
					<?php echo $form->search('keywords', $searchRequest['keywords'], array('placeholder' => t('Search Discounts')))?>
    ...
    <div class="ccm-dashboard-content-full marginTop10">
            ...
            <tbody>
                ...
                <?php if(count($discounts)>0) {
                    foreach ($discounts as $d) {
                        ?>
                        <tr>
                            <td><strong><a href="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/discounts/edit/', $d->drID)?>"><?= h($d->drName); ?></a></strong></td>
                            <td><?= h($d->drDisplay); ?></td>
                            <td>
                            ...
                            <td>
                                ...
                                if ($d->drValidFrom > 0) {
                                    $restrictions .= ' ' . t('starts') . ' : ' . $dfh->formatDateTime($d->drValidFrom) . '<br>';
                                }
                            ...
                            <td>
                                <p><a class="btn btn-default" href="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/discounts/edit/', $d->drID)?>"><i class="fa fa-pencil"></i></a></p>
                                <?php
                                if ($d->drTrigger == 'code') {
                                    echo '<p>' . '<a class="btn btn-default btn-sm" href="'.View::url('/'.$_SESSION['myURLTheme'].'/store/discounts/codes/', $d->drID).'">'.t('Manage Codes').'</a></p>';
                                } ?>
...
<?php if (in_array($controller->getTask(), $addViews)){ ?>
    ...
    <?php if ($controller->getTask() == 'edit') { ?>
        <div class="ccm-dashboard-header-buttons">
            <form method="post" id="deleterule" action="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/discounts/delete/')?>">
...
<!--        <p><em>To be implemented</em></p>-->
    ...
    <div class="ccm-dashboard-form-actions-wrapper">
        <div class="ccm-dashboard-form-actions">
            <a href="<?php echo URL::to('/'.$_SESSION['myURLTheme'].'/store/discounts')?>" class="btn btn-default"><?php echo t('Cancel')?></a>
...
<div class="ccm-dashboard-header-buttons">
    <a href="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/discounts/edit', $d->drID)?>" class="btn btn-default"><?php echo t("Edit Discount Rule")?></a>
</div>
...
<?php if (!empty($codes)) { ?>
            ...
            <?php foreach($codes as $code) { ?>
                    <?php if ($d->drSingleUseCodes) { ?>
                        ...
                        <?php } else { ?>
                            ...
                            <td>
                                <form method="post" action="<?= View::url('/'.$_SESSION['myURLTheme'].'/store/discounts/deletecode/')?>">
                    ...
                    <?php } else { ?>
                        <tr>
                            <td><?= $code['dcCode']; ?></td>
                            <td>
                                <form method="post" action="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/discounts/deletecode/')?>">
    ...
    <?php } ?>
<br />
<form method="post" action="<?php echo View::url('/'.$_SESSION['myURLTheme'].'/store/discounts/addcodes', $d->drID)?>" id="codes-add">
<fieldset><legend><?= t('Add Codes'); ?></legend>
    ...
    <div class="ccm-dashboard-form-actions-wrapper">
        <div class="ccm-dashboard-form-actions">
            <a href="<?php echo URL::to('/'.$_SESSION['myURLTheme'].'/store/discounts')?>" class="btn btn-default"><?php echo t('Return to Discount Rules')?></a>
        </div>
    </div>
