...
namespace Application\Controller\SinglePage\Account\Messages;
...
class Inbox extends AccountPageController
{
    ...
    public function view_message($box, $msgID)
    {
        ...
        if ($ui->canReadPrivateMessage($msg)) {
            ...
            $this->set('msg', $msg);
            $this->set('box', $box);
            $this->set('backURL', View::url('/'.$myURLAccount.'/messages/inbox', 'view_mailbox', h($box)));
            $valt = Loader::helper('validation/token');
            $token = $valt->generate('delete_message_' . $msgID);
            $this->set('deleteURL', View::url('/'.$myURLAccount.'/messages/inbox', 'delete_message', h($box), h($msgID), $token));
        } else {
    ...
    public function delete_message($box, $msgID, $token)
    {
        ...
        if ($ui->canReadPrivateMessage($msg) && (!$this->error->has())) {
            $msg->delete();
            $this->redirect('/'.$myURLAccount.'/messages/inbox', 'view_mailbox', $box);
        }
        print $this->view();
    }
    ...
    public function reply($boxID, $msgID)
    {
        ...
        $this->validateUser($uID);
        $this->set('backURL', View::url('/'.$myURLAccount.'/messages/inbox', 'view_message', $boxID, $msgID));
    ...
    public function send()
    {
        ...
        if ($vf->test()) {
            ...
            if ($r instanceof \Concrete\Core\Helper\Validation\Error) {
                ...
                if ($this->post('msgID') > 0) {
                    $this->redirect('/'.$myURLAccount.'/messages/inbox', 'reply_complete', $box, $msgID);
                } else {
                    $this->redirect('/'.$myURLAccount.'/messages/inbox', 'send_complete', $uID);
                }
            }
        } else {
