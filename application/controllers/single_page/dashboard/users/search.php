...
use \Application\Controller\Search\Users as SearchUsersController;
        ...
        switch ($_POST['task']) {
            case 'activate':
                ...
                if ($this->canActivateUser && Loader::helper('validation/token')->validate()) {
					...
					$this->redirect('/'.$myURLTheme.'/users/search', 'view', $this->user->getUserID(), 'activated');
                }
                break;
            case 'deactivate':
                $this->setupUser($uID);
                if ($this->canActivateUser && Loader::helper('validation/token')->validate()) {
					...
					$URLTheme = new URLTheme();
					$myURLTheme = $URLTheme->myURLTheme();
					$this->redirect('/'.$myURLTheme.'/users/search', 'view', $this->user->getUserID(), 'deactivated');
                }
                break;
            case 'validate':
                $this->setupUser($uID);
                if ($this->canActivateUser && Loader::helper('validation/token')->validate()) {
					...
					$URLTheme = new URLTheme();
					$myURLTheme = $URLTheme->myURLTheme();
					$this->redirect('/'.$myURLTheme.'/users/search', 'view', $this->user->getUserID(), 'email_validated');
                }
                break;
            case 'sudo':
                $this->setupUser($uID);
            ...
            case 'delete':
                ...
                if ($this->canDeleteUser && Loader::helper('validation/token')->validate()) {
					...
					$URLTheme = new URLTheme();
					$myURLTheme = $URLTheme->myURLTheme();
					$this->redirect('/'.$myURLTheme.'/users/search', 'delete_complete');
                }
                break;
        }
        $this->view($uID);
									...
									onLoad: function (concreteSearch) {
										concreteSearch.\$element.on('click', 'a[data-user-id]', function () {
											window.location.href='"
												. rtrim(URL::to('/'.$myURLTheme.'/users/search', 'view'), '/')
