...
//use \Application\Src\VividStore\Product\ProductFile as StoreProductFile;
...
class products extends DashboardPageController
{
    ...
    public function edit($pID, $status = '')
    {
        ...
        if (!$product) {
            ...
            $this->redirect('/'.$myURLTheme.'/store/products/');
        }
    ...
    public function generate($pID, $templateID=null)
    {
		...
		$URLTheme = new URLTheme();
		$myURLTheme = $URLTheme->myURLTheme();
        $this->redirect('/'.$myURLTheme.'/store/products/edit',$pID);
    }
    public function delete($pID)
    {
		...
		$URLTheme = new URLTheme();
		$myURLTheme = $URLTheme->myURLTheme();
        $this->redirect('/'.$myURLTheme.'/store/products/removed');
    }
    ...
    public function save()
    {
        ...
        if ($this->isPost()) {
            ...
            if (!$errors->has()) {
                ...
                if($data['pID']){
                    $this->redirect('/'.$myURLTheme.'/store/products/', 'updated');
                } else {
                    $this->redirect('/'.$myURLTheme.'/store/products/', 'success');
                }
    ...
    public function addgroup()
    {
        ...
        if (!$errors->has()) {
			...
			$URLTheme = new URLTheme();
			$myURLTheme = $URLTheme->myURLTheme();
            $this->redirect('/'.$myURLTheme.'/store/products/', 'groupadded');
        }
    }
	...
	public function savegroup()
	{
        ...
        //if ($this->isPost()) {
            //$errors = $this->validateGroup($args);
            //$this->error = $errors;
				...
				if(isset($_POST['addSectionCount'])){
					...
					if($addCount > 0){
						for($i=0;$i<$addCount;$i++){
							$groupName=mysql_real_escape_string($nameArr[$i]);
							
							$vals = array($groupName);
							$avID = $db->GetOne("SELECT gID FROM vividstoregroups WHERE groupName = ?", $vals);
							if ($avID == 0) {
								$vals1 = array($groupName);
								$db->Execute("INSERT INTO vividstoregroups(groupName) VALUES (?)", $vals1);
							}
						}
    ...
    /*public function validateGroup($args)
    {
		...
		//Multiple Valid
		$groupNameEnabled = 0;
		foreach ($args['name'] as $k => $v) {
			if (empty($v)) {
				$groupNameEnabled++;
			}
		}

        if($groupNameEnabled > 0){
